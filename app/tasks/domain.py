...
class DomainTask(CommonTask):
    ...
    def casm_search(self):
        casm_t1 = time.time()
        logger.info("start casm fetch {}".format(self.base_domain))
        casm_all_domains = utils.casm_domain(self.base_domain)
        domain_info_list = self.build_domain_info(casm_all_domains)
        if self.task_tag == "task":
            domain_info_list = self.clear_domain_info_by_record(domain_info_list)
            self.save_domain_info_list(domain_info_list, source=CollectSource.CASM)

        self.domain_info_list.extend(domain_info_list)
        elapse = time.time() - casm_t1
        logger.info("end casm fetch {} {} elapse {}".format(
    ...
    def domain_fetch(self):
        ...
        if self.options.get("casm_search"):
            self.update_task_field("status", "casm_search")
            t1 = time.time()
            self.casm_search()
            elapse = time.time() - t1
            self.update_services("casm_search", elapse)
