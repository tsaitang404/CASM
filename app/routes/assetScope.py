...
@ns.route('/')
class AssetScope(Resource):
    parser = get_parser(base_fields, location='args')
...
@ns.route('/delete/')
class DeleteAssetScope(Resource):
    parser = get_parser(delete_task_get_fields, location='args')
