...
@ns.route('/')
class CASMAssetScope(CASMResource):
    parser = get_casm_parser(base_fields, location='args')
...
@ns.route('/delete/')
class DeleteCASMAssetScope(CASMResource):
    parser = get_casm_parser(delete_task_get_fields, location='args')
...
@ns.route('/add/')
class AddCASMAssetScope(CASMResource):
    @auth
