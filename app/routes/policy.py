...
domain_config_fields = ns.model('domainConfig', {
    ...
    "casm_search": fields.Boolean(description="CASM 历史查询", default=True),
...
@ns.route('/add/')
class AddCASMPolicy(CASMResource):
    ...
    def post(self):
        ...
        if port_scan_type == "custom":
            ...
            port_list = utils.casm.build_port_custom(port_custom)
...
@ns.route('/delete/')
class DeletePolicy(CASMResource):
    @auth
...
def _update_plugin_config(config):
    ...
    for item in config:
        ...
        plugin_info = plugin_name_in_casm(plugin_name)
        if not plugin_info:
