...
class CASMSaveResultSet(CASMResource):
    parser = get_casm_parser(site_search_fields, location='args')
...
@ns.route('/add_tag/')
class AddSiteTagCASM(CASMResource):
...
@ns.route('/delete_tag/')
class DeleteSiteTagCASM(CASMResource):
...
@ns.route('/delete/')
class DeleteCASMSite(CASMResource):
    @auth
