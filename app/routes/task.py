...
from . import base_query_fields, CASMResource, get_casm_parser, conn
...
base_search_task_fields = {
    ...
    'options.casm_search': fields.Boolean(description="是否开启 CASM 历史查询"),
...
add_task_fields = ns.model('AddTask', {
    ...
    "casm_search": fields.Boolean(example=False),
...
@ns.route('/')
class CASMTask(CASMResource):
    parser = get_casm_parser(search_task_fields, location='args')
...
@ns.route('/batch_stop/')
class BatchStopTask(CASMResource):
...
class SyncTask(CASMResource):
    ...
    def post(self):
        ...
        celerytask.casm_task.delay(options=options)
...
# ******* 根据目标找到要同步的资产分组ID *********
@ns.route('/sync_scope/')
class Target2Scope(CASMResource):
    parser = get_casm_parser(sync_scope_fields, location='args')
...
# ******* 通过指定策略ID 下发任务 *********
@ns.route('/policy/')
class TaskByPolicy(CASMResource):
    @auth
...
# ******* 重新下发任务功能 *********
@ns.route('/restart/')
class TaskRestart(CASMResource):
    @auth
