...
from . import base_query_fields, Resource, get_parser, conn
...
base_search_task_fields = {
    ...
    'options.casm_search': fields.Boolean(description="是否开启 历史查询"),
...
@ns.route('/')
class Task(Resource):
    parser = get_parser(search_task_fields, location='args')
...
# ******* 根据目标找到要同步的资产分组ID *********
@ns.route('/sync_scope/')
class Target2Scope(Resource):
    parser = get_parser(sync_scope_fields, location='args')
