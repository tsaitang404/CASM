...
@ns.route('/')
class CASMScheduler(CASMResource):
...
@ns.route('/add/')
class AddCASMScheduler(CASMResource):
    ...
    def post(self):
        ...
        monitor_domain = utils.casm.get_monitor_domain_by_id(scope_id)
        scope_data = utils.casm.scope_data_by_id(scope_id)
...
@ns.route('/delete/')
class DeleteCASMScheduler(CASMResource):
...
# 下面有个批量恢复的，这个接口后面再删除
@ns.route('/recover/')
class RecoverCASMScheduler(CASMResource):
...
class BatchRecoverCASMScheduler(CASMResource):
...
# 下面有个批量停止的，这个接口后面再删除
@ns.route('/stop/')
class StopCASMScheduler(CASMResource):
...
@ns.route('/stop/batch')
class BatchStopCASMScheduler(CASMResource):
...
class AddSiteScheduler(CASMResource):
...
class AddWihScheduler(CASMResource):
