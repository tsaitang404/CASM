...
@ns.route('/delete/')
class DeleteScheduler(Resource):
...
# 下面有个批量恢复的，这个接口后面再删除
@ns.route('/recover/')
class RecoverScheduler(Resource):
...
class BatchRecoverScheduler(Resource):
...
@ns.route('/stop/batch')
class BatchStopScheduler(Resource):
...
class AddSiteScheduler(Resource):
...
class AddWihScheduler(Resource):
