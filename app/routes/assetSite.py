...
@ns.route('/delete/')
class DeleteAssetSite(Resource):
    @auth
...
class SaveResultSet(Resource):
    parser = get_parser(site_search_fields, location='args')
...
@ns.route('/add_tag/')
class AddAssetSiteTag(Resource):
...
@ns.route('/delete_tag/')
class DeleteAssetSiteTag(Resource):
