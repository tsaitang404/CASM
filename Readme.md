...
5. 加入指纹库(eHole`1017`、ehole_magic`24981`、EHole_magic_magic`25715`、FingerprintHub`2839`、dismap`4598`，去重后`21545`，可以手动更新，使用CASM导出的格式，有新指纹可以提issue，或者直接改json文件通过docker内源码安装)`docker快速安装没有ehole_magic相关指纹，去重后5k多，可以手动更新`
6. nmap使用最新版本(顺手的事，能yum装新的干嘛rpm装老的)
7. nuclei使用最新版本(通过github action每日更新)
8. CASM-NPoC、casm_files、geoip均移至tools目录下(建一堆项目太麻烦，且使用github action每日更新)
...
12. 提高工具并发数(大部分都是云服务器跑的，并发性能足够，已测试)`如果经常出现加入大量任务后超时的情况，请通过casm-worker.service降低并发数`
...
### Docker 安装（快速）
```bash
docker run --privileged -it -d -p 5003:5003 --name=casm --restart=always docker.adysec.com/adysec/casm /usr/sbin/init
...
cd /etc/systemd/system && systemctl restart casm*
exit
```
...
```bass
docker run --privileged -it -d -p 5003:5003 --name=casm --restart=always docker.adysec.com/library/centos /usr/sbin/init
docker exec -it casm bash
# docker内运行，通过源码安装CASM
curl https://raw.githubusercontent.com/adysec/CASM/master/misc/setup-casm.sh >install.sh
bash install.sh
exit
```
...
原版CASM仅适配centos 7，我更新至仅支持centos8（centos:latest）
如果在其他目录可以创建软连接，且安装了四个服务分别为`casm-web`, `casm-worker`, `casm-worker-github`, `casm-scheduler`

```
wget https://raw.githubusercontent.com/adysec/CASM/master/misc/setup-casm.sh
...
curl https://github.com/adysec/casm/raw/master/tools/smartdns.conf > /etc/smartdns/smartdns.conf
...
```
# 一键删站
docker stop casm && docker rm casm

# 删除镜像
docker rmi casm

# 改poc，poc位置/opt/CASM-NPoC
docker exec -it casm bash
...
11. 文件泄漏等风险检测
12. nuclei PoC 调用
13. [WebInfoHunter](https://tophanttechnology.github.io/CASM-doc/function_desc/web_info_hunter/) 调用和监控

### 截图

1. 登录页面     
   默认端口5003 (https), 默认用户名密码admin/casmpass  
...
| 8    | CASM历史查询    | 对casm历史任务结果进行查询用于本次任务                        |
